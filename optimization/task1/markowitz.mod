# From http://orfe.princeton.edu/~rvdb/ampl/nlmodels/markowitz/
...
param Cov {j in A, k in A} := sum {i in T} (Rtilde[i,j]*Rtilde[i,k]) / card(T);
...
#############
minimize lin_comb: mu * sum{i in T} (sum{j in A} Rtilde[i,j]*x[j])^2 / card{T} - sum{j in A} mean[j]*x[j];

### Alternative formulations
#minimize risk: sum{i in T} (sum{j in A} Rtilde[i,j]*x[j])^2 / card{T} ;
#subject to reward_bound: r_min <= sum{j in A} mean[j]*x[j];

#minimize reward: - sum{j in A} mean[j]*x[j] ;
#subject to risk_bound: sum{i in T} (sum{j in A} Rtilde[i,j]*x[j])^2 / card{T} <= s_max;

#minimize growth_rate: - sum{i in T} log (sum{j in A} x[j]*R[i,j]) / card{T};

subject to tot_mass: sum{j in A} x[j] = 1;
